...
function patientHasOpenLaserPackage(patient: Patient | null): boolean {
  ...
  for (const pkg of pkgs) {
    ...
    if (sessions.some((s) => !s.completedByReception && !s.linkedLaserSessionId)) return true
    if (sessions.some((s) => s.linkedLaserSessionId && !s.completedByReception)) return true
  }
  return false
}
...
export function ReceptionAppointmentPage() {
  ...
  async function submit(laserIntentArg?: LaserPackageBookingIntent): Promise<boolean> {
    setFormErr('')
    setSuccessMsg('')
    if (!picked) {
    ...
    if (needsPackageChoice && !laserIntent) {
  ...
  return (
    <>
      ...
      <p className="page-desc">
        {isDermatologyManager
          ? 'حجز موعد جلدية — ابحث عن المريض بالاسم، وإن لم يكن مسجّلاً يمكنك إنشاء ملف جديد ثم إكمال الحجز. يظهر الموعد في '
        ...
        {isDermatologyManager
          ? ' ليوم الموعد.'
          : ' ليوم الموعد. الأوقات الفارغة على شبكة كلّ ربع ساعة (09:00، 09:15، 09:30، …)، ويُضاف وقت بداية إضافي عند نهاية حجز لا يقع على حدّ الشبكة (مثلاً بعد موعد ينتهي 10:20)؛ بقية الأوقات تُحسب كما في المنطق السابق.'}
      </p>
            ...
            !laserPackageBookingIntent ? (
              <div
                ...
                style={{
                  ...
                  background: laserBookingContext?.partialVisit ? 'var(--warning-dim)' : 'var(--surface-1)',
                }}
              >
            ...
            <div className="card">
              ...
              {selectedService === 'laser' ? (
                <>
                  ...
                  (laserPackageBookingIntent === 'use_package' ||
                    laserPackageBookingIntent === 'continue_package' ||
                    !laserPackageBookingIntent) ? null : (
                    <>
                      {laserProcedureErr ? (
