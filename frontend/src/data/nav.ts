...
export const navItems: { key: NavKey; path: string; label: string }[] = [
  ...
  { key: 'admin_expenses', path: '/admin/expenses', label: 'المصاريف' },
  { key: 'admin_salaries', path: '/admin/salaries', label: 'الرواتب' },
  { key: 'general_finance_dashboard', path: '/admin/finance-dashboard', label: 'لوحة المالية العامة' },
...
export function roleLabel(role: Role): string {
  const map: Record<Role, string> = {
    ...
    dental_assistant: 'مساعدين أسنان',
  }
...
export function canAccessTab(role: Role, tab: 'laser' | 'dermatology' | 'dental'): boolean {
  ...
  if (role === 'dental_branch' || role === 'dental_assistant') return tab === 'dental'
  return false
}
